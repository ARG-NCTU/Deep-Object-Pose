...
# load config
with open('./config/param.yaml', 'r') as f:
    config = yaml.load(f)
...
if not "/" in config['outf']:
    config['outf'] = "train_{}".format(config['outf'])
...
with open (config['outf']+'/header.txt','w') as file: 
    file.write(str(opt)+"\n")

with open (config['outf']+'/header.txt','w') as file: 
    file.write(str(opt))
    file.write("seed: "+ str(opt.manualseed)+'\n')
    with open (config['outf']+'/test_metric.csv','w') as file:
...
if opt.save:
    for i in range(2):
        ...
        if normal_imgs is None:
            normal_imgs = [0,1]
        save_image(images['img'],'{}/train_{}.png'.format( config['outf'],str(i).zfill(5)),mean=normal_imgs[0],std=normal_imgs[1])

        print (i)        

    print ('things are saved in {}'.format(config['outf']))
    quit()
...
net = torch.nn.DataParallel(net,device_ids=config['gpuids']).cuda()
...
with open (config['outf']+'/loss_train.csv','w') as file: 
    file.write('epoch,batchid,loss\n')

with open (config['outf']+'/loss_test.csv','w') as file: 
    file.write('epoch,batchid,loss\n')
...
def _runnetwork(epoch, loader, train=True):
    ...
    for batch_idx, targets in enumerate(loader):
        ...
        with open (config['outf']+namefile,'a') as file:
            s = '{}, {},{:.15f}\n'.format(
        ...
        if not opt.nbupdates is None and nb_update_network > int(opt.nbupdates):
            torch.save(net.state_dict(), '{}/net_{}.pth'.format(config['outf'], config['namefile']))
            break
...
for epoch in range(1, config['epochs'] + 1):
    ...
    try:
        torch.save(net.state_dict(), '{}/net_{}_{}.pth'.format(config['outf'], config['namefile'] ,epoch))
    except:
        pass
